...
class TapinTrend extends Component {
    // const [, setmocsummarry] = useState({});
    constructor(props) {
        super(props);
        this.state = { data1: [], data2: [], label: [] };
    }
    componentDidMount() {
        const url = 'http://localhost:8080/ChargeableEventGeneration/tapin/moctrendanalysis';
        axios.get(url).then((response) => {
            ...
            response.data.map((dat) => {
                data1.push(dat['maxMOCCOUNT']);
                data2.push(dat['maxMOCTOTALSDR']);
                label.push(dat['_id']);
            });
            ...
            >
                ...
                <LineChart
                    option={{
                        ...
                        //   },
                        legend: {
                            data: ['Max MOC Count', 'Max MOC total SDR'],
                            orient: 'vertical',
                            top: 18,
                            right: 10
                        },
