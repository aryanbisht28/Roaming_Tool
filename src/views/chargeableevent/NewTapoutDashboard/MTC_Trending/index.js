...
function Index() {
    ...
    const [mtcsummarry, setmtcsummarry] = useState({});
    ...
    const [marks, setValue] = useState([0, 10]);
    ...
    useEffect(() => {
        const url = 'http://localhost:8080/ChargeableEventGeneration/countadig';
        axios.get(url).then((response) => {
            ...
            let dateRange = [];
            response.data[2].map((val) => {
                console.log('val', val);
                // let obj = {};
                // obj['value'] = val['_id'].split('T')[0];
                // obj['label'] = `${val['_id'].split('T')[0]}`;
                dateRange.push(val['_id'].split('T')[0]);
                // response.data[2].map((dat) =>
                // console.log('date', dat)
            });
            //     dateRange.push(response.data[2][0]['_id'].split('T')[0])
            //     dateRange.push(response.data[2][response.data[2].length - 1]['_id'].split('T')[0]);
            console.log('range', dateRange);
            setValue(dateRange);
        });
        const url1 = 'http://localhost:8080/ChargeableEventGeneration/mtcsummary';
        axios.get(url1).then((response) => {
            setmtcsummarry(response.data);
        });
    }, []);
    ...
    const handleCountry = async (event) => {
        ...
        try {
            if (event.target.value === 'clear') {
                // window.location.href = '/pages/ChargeableEventGenerationdash';
                handleClick();
            } else {
                setCountry(event.target.value);
                let data = {};
                data['country'] = event.target.value;
                const url = 'http://localhost:8080/ChargeableEventGeneration/mtcfiltercountry';
    ...
    const handleTadig = async (event) => {
        try {
            ...
            const url = 'http://localhost:8080/ChargeableEventGeneration/mtcfiltertadig';
            const { data: res } = await axios.post(url, data);
        ...
        <Grid container spacing={gridSpacing} xs={12}>
            <Grid container spacing={gridSpacing} sx={{ mt: 1, ml: 0 }} xs={12}>
                <Grid item xs={4}>
                    ...
                    >
                        <h3 style={{ margin: '0.2rem' }}>MTC Count</h3>
                        <div
                            style={{
                                width: '95%',
                                height: '1.5px',
                        ...
                        >
                            ...
                            <h4>{Object.keys(mtcsummarry).length != 0 ? mtcsummarry['MTCCOUNT'][0]['MTCCOUNT'] : '0'}</h4>
                        </div>
                    </Card>
                </Grid>
                <Grid item xs={4}>
                    ...
                    >
                        <h3 style={{ margin: '0.2rem' }}>MTC Total SDR</h3>
                        <div
                            style={{
                                width: '95%',
                                height: '1.5px',
                        ...
                        >
                            ...
                            <h4>{Object.keys(mtcsummarry).length != 0 ? mtcsummarry['MTCTOTALSDR'][0]['MTCTOTALSDR'] : '0'}</h4>
                        </div>
                    </Card>
                </Grid>
                <Grid item xs={4}>
            ...
            <Grid container spacing={gridSpacing} xs={12} sx={{ mt: -2, ml: 0 }}>
                <Grid item xs={4}>
                    ...
                    >
                        <h3 style={{ margin: '0.2rem' }}>MTC Actual Duration</h3>
                        <div
                            style={{
                                width: '95%',
                                height: '1.5px',
                        ...
                        >
                            ...
                            <h4>{Object.keys(mtcsummarry).length != 0 ? mtcsummarry['mtcactualduration'][0]['totalduration'] : '0'}</h4>
                        </div>
                    </Card>
                </Grid>
                <Grid item xs={4}>
                    ...
                    >
                        ...
                        >
                            ...
                            <h4>{Object.keys(mtcsummarry).length != 0 ? mtcsummarry['tap'] : '0'}</h4>
                        </div>
                    </Card>
                </Grid>
                <Grid item xs={4}>
