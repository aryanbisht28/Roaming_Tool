...
import Smsmocount from './Smsmocount';
...
function Index() {
    ...
    const [smsmosummary, setsmsmosummary] = useState({});
    ...
    useEffect(() => {
        ...
        const url1 = 'http://localhost:8080/ChargeableEventGeneration/smsmosummary';
        axios.get(url1).then((response) => {
            // console.log('Get req', response.data[0], response.data[1]);
            setsmsmosummary(response.data);
        });
    }, []);
    ...
    const handleCountry = async (event) => {
        ...
        try {
            if (event.target.value === 'clear') {
                ...
                const url = 'http://localhost:8080/ChargeableEventGeneration/smsmofiltercountry';
    ...
    const handleTadig = async (event) => {
        try {
            ...
            const url = 'http://localhost:8080/ChargeableEventGeneration/smsmofiltertadig';
            const { data: res } = await axios.post(url, data);
        ...
        <Grid container spacing={gridSpacing} xs={12}>
            <Grid container spacing={gridSpacing} sx={{ mt: 1, ml: 0 }} xs={12}>
                <Grid item xs={4}>
                    ...
                    >
                        <h3 style={{ margin: '0.2rem' }}>SMSMO Total SDR </h3>
                        <div
                            style={{
                                width: '95%',
                                height: '1.5px',
                        ...
                        >
                            ...
                            <h4>{Object.keys(smsmosummary).length != 0 ? smsmosummary['smsmototalsdr'][0]['totalduration'] : '0'}</h4>
                        </div>
                    </Card>
                </Grid>
                <Grid item xs={4}>
                    ...
                    >
                        <h3 style={{ margin: '0.2rem' }}>SMSMO Count</h3>
                        <div
                            style={{
                                width: '95%',
                                height: '1.5px',
                        ...
                        >
                            ...
                            <h4>{Object.keys(smsmosummary).length != 0 ? smsmosummary['SMSMOCOUNT'][0]['SMSMOCOUNT'] : '0'}</h4>
                        </div>
                    </Card>
                </Grid>
                <Grid item xs={4}>
                    ...
                    >
                        ...
                        >
                            ...
                            <h4>{Object.keys(smsmosummary).length != 0 ? smsmosummary['tap'] : '0'}</h4>
                        </div>
                    </Card>
                </Grid>
            </Grid>
