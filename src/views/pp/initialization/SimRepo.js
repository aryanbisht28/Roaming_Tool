...
function SimRepo({ roam, service, direction, unilateral, spoc }) {
    ...
    const postReQ = async (own, arrroam) => {
        try {
            ...
            data['name'] = `${localStorage.getItem('firstname')} ${localStorage.getItem('lastname')}`;
    ...
    return (
        <>
            <fieldset style={{ background: '#ffff', marginTop: '1.3em' }}>
                <legend style={{ fontSize: '1.3rem', fontWeight: '500' }}>SIM Repository</legend>
                <div style={{ height: 290, width: '100%', marginTop: '.5em' }}>
                    <DataGrid
